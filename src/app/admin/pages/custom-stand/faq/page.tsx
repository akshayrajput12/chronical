...
const CustomStandFAQEditor = () => {
  ...
  const handleUpdateItem = (index: number, field: keyof CustomExhibitionFAQItem, value: any) => {
  ...
  const handleUpdateListItem = (itemIndex: number, listIndex: number, value: string) => {
    setFaqItems(prev => prev.map((item, i) => 
      i === itemIndex 
        ? { 
            ...item, 
            list_items: item.list_items?.map((listItem, li) => 
  ...
  const handleRemoveListItem = (itemIndex: number, listIndex: number) => {
    setFaqItems(prev => prev.map((item, i) => 
      i === itemIndex 
        ? { 
            ...item, 
            list_items: item.list_items?.filter((_, li) => li !== listIndex) || []
          }
        : item
    ));
  };
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">FAQ Section</h1>
          ...
          <div className="space-y-4">
            {faqItems.map((item, index) => (
              <FAQItemEditor
                ...
                onUpdateListItem={(listIndex, value) => handleUpdateListItem(index, listIndex, value)}
                onRemoveListItem={(listIndex) => handleRemoveListItem(index, listIndex)}
              />
            ))}
          </div>
        </motion.div>
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                {item.list_items?.map((listItem, listIndex) => (
                  <div key={listIndex} className="flex items-center space-x-2 mb-2">
                    <input
                      type="text"
                      value={listItem}
                      onChange={(e) => onUpdateListItem(listIndex, e.target.value)}
                      className="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      placeholder="Enter list item"
                    />
                    <button
                      onClick={() => onRemoveListItem(listIndex)}
