...
const EssentialSupport = () => {
  ...
  const [sectionData, setSectionData] = useState<EssentialSupportSection | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (data) {
          console.log('Essential support section data received:', data);
          setSectionData(data);
        } else {
  ...
  const getSortedCategories = () => {
    if (sectionData?.categories) {
      return [...sectionData.categories].sort((a, b) => a.display_order - b.display_order);
    }
    return [];
  };

  // Get sorted services for a category
  const getSortedServices = (category: { services?: Array<{ service_text: string; display_order: number; is_active?: boolean }> }) => {
    if (category.services) {
      return [...category.services].sort((a, b) => a.display_order - b.display_order);
    }
    return [];
  };
      ...
      <section className="py-20 bg-gray-100 overflow-hidden relative" ref={ref}>
        <div className="container mx-auto px-4 text-center">
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-gray-600">Loading essential support services...</p>
          </div>
        </div>
      </section>
    );
  ...
  if (error || !sectionData || categories.length === 0) {
    return (
      <section className="py-20 bg-gray-100 overflow-hidden relative" ref={ref}>
        <div className="container mx-auto px-4 text-center">
          <p className="text-red-500 mb-2">{error ? "Error loading content" : "No content available"}</p>
          <p className="text-gray-600">Essential support services are currently unavailable. Please check the admin panel to ensure data has been added.</p>
        </div>
      </section>
    );
  }
          ...
          <div className="flex justify-start mb-8">
            <div className="flex flex-col bg-white p-6 shadow-md hover:shadow-lg transition-shadow duration-300 w-full md:w-1/2 lg:w-1/4">
              <div className="flex items-center mb-6">
                <div className="w-14 h-14 rounded-full border-2 border-[#a5cd39] flex items-center justify-center text-[#a5cd39]">
                  <div className="w-6 h-6">
                    {renderSvgIcon(categories[4].icon_svg)}
                  </div>
                </div>
                <div className="border-t border-gray-300 flex-grow ml-4"></div>
              </div>
