...
export default function MinimalLoaderDemoPage() {
  const { showLoader, showInlineLoader, showCenterLoader, hideLoader, isLoading } = useMinimalLoader();
  ...
  const simulateDataFetch = async (delay: number = 2000) => {
    const result = await fetchWithLoading(
      () => new Promise<string>(resolve => 
        setTimeout(() => resolve(`Data loaded at ${new Date().toLocaleTimeString()}`), delay)
      ),
      "Fetching data..."
    );
    setData(result);
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 space-y-8">
            <div className="text-center">
              <p className="text-gray-600 mb-6">
                Test the minimal, non-intrusive loader that doesn't block the entire screen.
              </p>
            </div>

            {/* Position Demos */}
            <div className="space-y-4">
              <h3 className="text-lg font-semibold">Position Demos</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <Button
                  onClick={() => {
                    showLoader("Top Right", { position: "top-right", showMessage: false });
                ...
                <Button
                  onClick={() => {
                    showLoader("Bottom Right", { position: "bottom-right", showMessage: false });
                    setTimeout(hideLoader, 3000);
                  }}
                  disabled={isLoading}
            ...
            {/* Size Demos */}
            <div className="space-y-4">
              <h3 className="text-lg font-semibold">Size Demos</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Button
                  onClick={() => {
                    showLoader("Tiny loader", { size: "tiny", position: "top-right" });
                    setTimeout(hideLoader, 3000);
                  }}
                  disabled={isLoading}
                ...
                <Button
                  onClick={() => {
                    showLoader("Small loader", { size: "small", position: "top-right" });
                    setTimeout(hideLoader, 3000);
                  }}
                  disabled={isLoading}
                ...
                <Button
                  onClick={() => {
                    showLoader("Medium loader", { size: "medium", position: "center", showMessage: true });
                    setTimeout(hideLoader, 3000);
                  }}
                  disabled={isLoading}
            ...
            {/* Data Loading Demo */}
            <div className="space-y-4">
              <h3 className="text-lg font-semibold">Data Loading Demo</h3>
            ...
            <div className="bg-gray-50 rounded-lg p-6">
              <h3 className="text-lg font-semibold mb-4">Minimal Loader Features:</h3>
              <ul className="space-y-2 text-gray-600">
                <li>✅ Small, non-intrusive design</li>
                <li>✅ Doesn't block the entire screen</li>
                <li>✅ Multiple size options (tiny, small, medium)</li>
                <li>✅ Flexible positioning (corners, center, inline)</li>
                <li>✅ Optional loading messages</li>
                <li>✅ Smooth animations</li>
                <li>✅ Perfect for data loading without blank screens</li>
                <li>✅ Uses website logo for brand consistency</li>
              </ul>
            </div>
